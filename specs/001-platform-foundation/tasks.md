...
- [x] T069 Run the whole of [quickstart.md](./quickstart.md) from a fresh clone on a clean Docker state and fix anything that does not match ✅ Ran from a clone on wiped volumes. Stack up in 34s cold / 7.5s warm, migrations to head, pgvector 0.8.6 and the MinIO bucket created on first boot, fail-fast on a missing `SESSION_SECRET`, Redis outage naming `cache`, all four backend gates green (55 tests, 89%), 6/6 Playwright, and sign-in taking the database from `0|0` to `1|1` with a second sign-in leaving it at `1|1` and only advancing `last_login_at`. **Four documentation defects found and fixed**: the User Story 3 gates were documented as `docker compose exec`, but `tests/` is dockerignored so `docker compose exec backend pytest` collects nothing and exits looking like a pass, and the frontend container is `target: dev` so a build inside it races the dev server for `/app/.next` and fails — both now documented as host commands matching CI, with `npm run typecheck` added; the Common Issues list ended with a duplicate port entry contradicting the first; `name: careerhq` is pinned so a fresh clone silently inherits the previous database; and the README's first-time setup hardcoded a macOS-only Python path
