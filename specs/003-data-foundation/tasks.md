...
- **T031** — the import-graph test. Principle V holds only while `litellm_gateway.py` is the sole
  importer of `litellm`. Reviewer vigilance is not a mechanism; a test is.
- **T045** — the suite passing with **no API key**, verified by unsetting it rather than by
  assuming the fake was used.
- **T063** — the absence of a `rejected` column. Enforcement is a thing *not* existing, so nothing
...
- [ ] T012 Confirm `ruff check`, `ruff format --check`, `mypy src` and `pytest` all pass from
...
- [ ] T013 [P] [US1] Test in `backend/tests/unit/test_ports.py`: `StructuredCompletion.complete`
...
- [ ] T014 [P] [US1] Test in `backend/tests/unit/test_ports.py`: the returned `Completion.usage`
      carries model, input tokens, output tokens and cost. **Correct red**: `ImportError` (O4,
      FR-026)
- [ ] T015 [P] [US1] Test in `backend/tests/unit/test_extraction_gateway.py`: output failing schema
      validation raises rather than returning partial data. **Correct red**: `ImportError` (O2,
      FR-025)
- [ ] T016 [P] [US1] Test in `backend/tests/unit/test_extraction_gateway.py`: model selection
...
- [ ] T017 [P] [US1] Test in `backend/tests/unit/test_extraction_gateway.py`: `FixtureGateway`
      returns `usage.is_fixture = True`, and the real adapter returns `False`. **Correct red**:
      `ImportError` (R3)

### The seam — implementation

- [ ] T018 [US1] Create `backend/src/careerhq/application/ports.py` with the
...
- [ ] T019 [US1] Create `backend/src/careerhq/infrastructure/ai/litellm_gateway.py` implementing the
      Protocol over LiteLLM. **This must be the only module in the codebase that imports
      `litellm`** (O5)
- [ ] T020 [P] [US1] Create `backend/src/careerhq/infrastructure/ai/fixture_gateway.py` returning
...
- [ ] T021 [US1] Add `get_structured_completion` to `backend/src/careerhq/api/deps.py`, resolving
...
- [ ] T022 [P] [US1] Test in `backend/tests/unit/test_documents.py`: text is recovered from the
      sample PDF and DOCX fixtures. **Correct red**: `ImportError`
- [ ] T023 [P] [US1] Test in `backend/tests/unit/test_documents.py`: a PDF with no text layer
      yields empty text and is reported as such, **not** as an empty successful extraction (FR-008)
- [ ] T024 [P] [US1] Create `backend/src/careerhq/infrastructure/documents/pdf.py` using pdfplumber
- [ ] T025 [P] [US1] Create `backend/src/careerhq/infrastructure/documents/docx.py` using python-docx

### Data model

- [ ] T026 [US1] Define the extraction schema in
...
- [ ] T029 [US1] Write migration `backend/alembic/versions/0003_*.py` for the staging and profile
      tables, including **constraint C4** — `UNIQUE (profile_id) WHERE is_master`

### Behaviour — tests first

- [ ] T030 [P] [US1] Test in `backend/tests/integration/test_import_flow.py`: after upload and
...
- [ ] T031 [US1] Test in `backend/tests/unit/test_architecture.py`: **`litellm` is imported by
...
- [ ] T032 [P] [US1] Test in `backend/tests/integration/test_import_flow.py`: approving creates
...
- [ ] T033 [P] [US1] Test in `backend/tests/integration/test_import_flow.py`: a corrected item is
...
- [ ] T034 [P] [US1] Test in `backend/tests/integration/test_import_flow.py`: an abandoned import
      leaves the profile empty and is discardable (Scenario 6, FR-007)
- [ ] T035 [P] [US1] Test in `backend/tests/integration/test_import_flow.py`: a file that is
      neither PDF nor DOCX is rejected with a message naming accepted formats, and nothing is
      stored (FR-001, Scenario 5)
- [ ] T036 [P] [US1] Test in `backend/tests/integration/test_import_flow.py`: extraction yielding
      nothing returns 422 with `extraction_error` set — **not** 202 with an empty item list
      (FR-008)
- [ ] T037 [P] [US1] Test in `backend/tests/integration/test_import_flow.py`: a second import for a
...
- [ ] T038 [P] [US1] Test in `backend/tests/integration/test_import_flow.py`: a high confidence
...
- [ ] T039 [P] [US1] Test in `backend/tests/integration/test_import_flow.py`: a failed approval
      leaves nothing behind — no partial profile, no orphaned Master Resume (FR-023)
- [ ] T040 [P] [US1] Extend the route-enumeration test in
...
- [ ] T041 [US1] Create `backend/src/careerhq/application/extract_resume.py`: store the upload,
      extract text, call the seam, persist the staged `ImportedResume` and its items with the usage
      record
- [ ] T042 [US1] Create `backend/src/careerhq/application/approve_import.py`: write accepted items
      to the profile and create the Master Resume **in one transaction** (FR-023, R6)
- [ ] T043 [US1] Create `backend/src/careerhq/api/routes/imports.py` with the endpoints in
      [contracts/http-api.md](./contracts/http-api.md), including the 503 that names the missing
      provider setting (FR-028)
- [ ] T044 [US1] Confirm backend gates pass and coverage holds at ≥80%

### The check that keeps the suite honest

- [ ] T045 [US1] **Run the full suite with `ANTHROPIC_API_KEY` and `AI_PROVIDER` unset**, and
...
- [ ] T046 [P] [US1] Build the upload screen and its states in
...
- [ ] T047 [US1] Build the review interface in `frontend/src/components/import-review/` per
...
- [ ] T048 [P] [US1] Implement the provenance and confidence primitives in
      ...
      carrier
- [ ] T049 [P] [US1] Keyboard navigation for the review list: `A` accept, `E` edit, `D` discard,
      `J`/`K` to move. This is the difference between reviewing sixty items and abandoning the
      import, not a nicety (docs/09 §6.5)
- [ ] T050 [P] [US1] Persistent fixture banner in `--color-fixture` whenever `is_fixture` is set,
      visible for the whole review. The one unacceptable outcome is approving invented content into
      a real profile (R3)
- [ ] T051 [P] [US1] Component tests in `frontend/src/components/__tests__/` for the provenance
      primitives and the review item states
- [ ] T052 [US1] Build the Profile screen at `frontend/src/app/profile/page.tsx` per docs/09 §6.4,
...
- [ ] T053 👁 **OBSERVE** [US1] Run the quickstart's User Story 1 walkthrough against
...
- [ ] T054 👁 **OBSERVE** [US1] Upload a scanned/image-only PDF against the running stack and
      confirm it reads as a **failure**, not an empty form (FR-008, docs/09 §5)
- [ ] T055 [US1] Measure SC-002 against a real CV: at least 80% of work-experience bullets
...
- [ ] T056 [P] [US2] Test in `backend/tests/integration/test_applications.py`: an application is
      valid with **no** submitted resume while in a pre-submission status (FR-011)
- [ ] T057 [P] [US2] Test in `backend/tests/integration/test_applications.py`: two users cannot see
...
- [ ] T058 [P] [US2] Test in `backend/tests/integration/test_applications.py`: every status change
      writes a history row, and there is **no update or delete path** to that table (FR-012,
      constraint C6, Constitution IV)
- [ ] T059 [P] [US2] Test in `backend/tests/integration/test_applications.py`: two applications
      naming the same company resolve to **one** company row (FR-014, constraint C2)
- [ ] T060 [P] [US2] Test in `backend/tests/integration/test_applications.py`: `normalized_status`
...
- [ ] T061 [US2] Add `Application`, `Company` and `ApplicationStatusHistory` to
      ...
      interchangeably (R8)
- [ ] T062 [US2] Write migration `backend/alembic/versions/0004_*.py` with constraints **C2**
      (`UNIQUE (user_id, normalized_name)` on companies) and **C3** (partial
      `UNIQUE (user_id, import_source, import_source_id)` on applications)
- [ ] T063 [US2] Test in `backend/tests/integration/test_applications.py`: **no column named
...
- [ ] T064 [US2] Create `backend/src/careerhq/application/record_application.py` and
      `backend/src/careerhq/api/routes/applications.py` per contracts/http-api.md
- [ ] T065 [P] [US2] Build the applications table at `frontend/src/app/applications/page.tsx` per
...
- [ ] T066 [P] [US2] Build the dashboard at `frontend/src/app/dashboard/page.tsx` per docs/09 §6.1,
...
- [ ] T067 [US2] Build the tabbed application detail at
...
- [ ] T068 [P] [US2] Implement the *not built yet* tab treatment — unbuilt capabilities marked in
...
- [ ] T069 👁 **OBSERVE** [US2] Against the running stack: create an application with a real job
...
- [ ] T070 🔧 **MANUAL** [US3] Export real data from JobTracker (`GET /api/export`) and add it to
...
- [ ] T071 [US3] Test in `backend/tests/integration/test_jobtracker_import.py`: **idempotency
...
- [ ] T072 [P] [US3] Test in `backend/tests/unit/test_jobtracker_mapping.py`: a row with
...
- [ ] T073 [P] [US3] Test in `backend/tests/unit/test_jobtracker_mapping.py`: an unrecognised
...
- [ ] T074 [P] [US3] Test in `backend/tests/unit/test_jobtracker_mapping.py`: dates parse
...
- [ ] T075 [P] [US3] Test in `backend/tests/integration/test_jobtracker_import.py`: unmappable rows
...
- [ ] T076 [P] [US3] Test in `backend/tests/unit/test_jobtracker_mapping.py`: the source
...
- [ ] T077 [US3] Create `backend/src/careerhq/application/import_jobtracker.py` — validate and
      partition rows **before** opening the transaction, then import in one commit
- [ ] T078 [US3] Add the import endpoint to `backend/src/careerhq/api/routes/applications.py`
      returning the imported / skipped / rejected report
- [ ] T079 [P] [US3] Build the import screen and its report at
      `frontend/src/app/applications/import/page.tsx`
- [ ] T080 👁 **OBSERVE** [US3] Import the real export against the running stack. Confirm counts,
...
- [ ] T081 🔧 **MANUAL** Create the Railway bucket and set credentials: `railway bucket create`,
...
- [ ] T082 🔧 **MANUAL** Set `AI_PROVIDER` and `ANTHROPIC_API_KEY` on the deployed backend service
- [ ] T083 👁 **OBSERVE** `curl` deployed `/api/health/ready` and confirm **`object_storage` and
...
- [ ] T084 👁 **OBSERVE** Import a CV on the **deployed** site and confirm the file landed in the
...
- [ ] T085 👁 **OBSERVE** Run T063's `information_schema` query against the **deployed** database.
      A constraint that holds locally and not in production is not a constraint

### Gates and documentation

- [ ] T086 Run every gate from the host and confirm green: `ruff format --check .`, `ruff check .`,
...
- [ ] T087 [P] Update `README.md` with the CV import flow, the provider-key options, and the bucket
      requirement
- [ ] T088 [P] Update `CLAUDE.md`: slice 003 complete, what carries into slice 004, and any gotcha
      this slice proved with its symptom
- [ ] T089 [P] Update `docs/05_Implementation_Plan.md` and `docs/08_Technical_Spec.md` — slice 003
      complete, slice 004 next, capability status markers moved
- [ ] T090 Walk `specs/003-data-foundation/quickstart.md` end to end **as written** and correct it
...
- [ ] T091 Confirm the scope guards held: **exactly one model call** in the whole slice, no agent
...
Phase 4 US2 (P2)  ─── slice 004's job-description input
    ↓
Phase 5 US3 (P3)  ─── history for slice 007; needs T070's real export
    ↓
Phase 6 Polish
```
...
- **Phase 3 seam tests**: T013–T017 together
- **Phase 3 behaviour tests**: T030, T032–T040 together
- **Phase 3 frontend**: T046, T048, T049, T050, T051 alongside backend work
- **Phase 4 tests**: T056–T060 together
- **Phase 5 mapping tests**: T072–T076 together after T071
- **Phase 6 docs**: T087, T088, T089 together
