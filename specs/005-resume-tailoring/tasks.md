...
- [X] T085 Measure a real run on a real posting — both the first-pass-clear and full-revision-budget paths — and record tokens, cost and elapsed time in `research.md` R5, the way slice 004's R8 recorded its own **Done**: both paths measured on real postings — full revision budget (Harman `60263226`, 7 calls, $0.547891, 4m01s, escalation fired) and first-pass clear (Voyantis `ff0e310c`, 3 calls, $0.307106, 3m43s; Cellebrite `2615363e`, $0.295450, 2m50s). Per-call breakdowns recorded in research.md R5 → *T085*, which T092 made possible. **No new run was needed** — the evidence already existed
- [X] T086 Compare the measurement against SC-006 ($0.30) and SC-001 (90s / 3min). **If missed, mark missed in `spec.md`** rather than adjusting the target — slice 004 did exactly this with SC-004 **Done**: SC-006 met by one run of four (worst case 1.83x); SC-001 missed by all four on both clauses. Both marked **missed** in `spec.md` with the measurements, and **neither target adjusted** — the SC-004 precedent
- [X] T087 Read a tailored resume as a person and ask whether it claims anything the owner did not do. FR-017 has no test that answers this **Done**: the Harman resume read beside the profile. Nothing in the final document claims what the profile does not support; three grounding problems arose and all three were corrected — one `ungrounded` discarded with the owner's wording restored character-identically, two `overstated` reverted during revision. Recorded in research.md R5 → *T087*, labelled one reader / one run / not a measurement
- [x] T088 Deploy and verify on the live system: a real run with `is_fixture = false`, a real model name, real token counts, a real cost — **PARTIALLY DONE, deliberately left open. Infrastructure verified 2026-08-26**: PR #2 merged as `a41e675`, Railway deployment `SUCCESS` (held at `WAITING` until CI passed, as the slice-002 gate intends), production schema stepped **0009 → 0014** with all five tailoring tables and both Phase 7 columns present, readiness reporting `database ok · cache not_configured · object_storage ok · ai_provider ok`, all six tailoring routes in the deployed OpenAPI and returning 401 unauthenticated, and production data byte-identical to the pre-deploy baseline. **The acceptance criterion this task is actually about — a real run with `is_fixture = false`, a real model name, real token counts and a real cost — was NOT met at that point**, because a paid production run was deliberately deferred by decision. The criterion was left exactly as written and the task stayed open until a real run was made. ***It was made on 2026-08-30 and the criterion is now met in full***: run `f116683f`, `succeeded`, **`is_fixture = false`**, **35,785 input / 18,808 output tokens** across three calls, **$0.312825**, 3m 15.9s, with real model names recorded per task — Plan and Draft on `anthropic/claude-sonnet-5`, Review on **`anthropic/claude-opus-5`** (its configured model, not an escalation). `attempts = 0`, confidence 84, one `overstated` and six `uncovered` findings and **no `ungrounded`**, so the discard path still has no production exercise. `guidelines_used` holds **28 real corpus citations across 9 documents** whose content hashes all resolve to live `knowledge_chunks` — the first production proof that retrieval works. ***The tick was withheld pending the run, not lost***: every field above was re-read from the deployed `tailoring_runs` row on 2026-08-31 before ticking
- [X] T089 Query the deployed database with the mandatory `PGHOST=localhost PGPORT=5432` override — the running container carries a stale host from a deleted proxy, and Railway recycles those ports, so the default address now serves another tenant's database — **Done**: used throughout the deployment verification. The override was again not optional; the deployed schema version, the five new tables, both Phase 7 columns, and every production row count were read this way
...
- [X] T095 Record which items a Draft proposal actually touched, so a reorder stops reading as inaction. **Origin**: the Voyantis run `ff0e310c` reordered all nine experience bullets and ten of eleven skills while carrying only three text proposals, and `plan_execution` classified a planned emphasis the Draft demonstrably moved (`d61a73e8`) as `no_evidence`. Root cause: `text=None` is deliberately overloaded — `finalisation_rules.finalise`'s own docstring says "one representation of 'no change', not two" — and persistence writes `position=proposal.position if proposal else master_item["position"]`, overwriting the master position with no record that it was overwritten. **Acceptance**: a nullable `displaced_position` column on `resume_version_items` (migration `0014_displaced_position`, chained on `0013_review_passes`), written by the persistence loop with the master position **only when a proposal arrived** and left NULL otherwise, so the master's ordering at creation is `COALESCE(displaced_position, position)` for every item — closing the **FR-030** gap that today loses it for the 13 items carrying a proposal; `plan_execution` gains the states `reordered` (a proposal arrived and moved the item) and `unknown_position` (NULL on a row written before this column existed — **never** silently read as untouched), **D1 counts `reordered` as acted**, **D3 is unchanged** because a reorder is not a surviving text change; **no backfill of historical rows** — the action half is unknowable and derived master positions must not be presented as record; **no change to `DraftedItem` or any model-facing schema, no prompt, threshold, model-configuration, workflow or finalisation-semantics change, and no provider call**. Test-first, with the `d61a73e8` case as a watched-failing regression. **Semantics resolved before implementation**: four no-text states — `no_evidence` (no proposal arrived, on a row where that is knowable), `proposed` (a proposal arrived and the position did not change), `reordered` (a proposal arrived and the position changed), `unknown_position` (a row written before the column existed, where proposal arrival cannot be reconstructed). **D1 `acted` includes `proposed` and `reordered`; `unknown_position` is excluded from D1's denominator and its count reported separately.** **D3 is untouched** — its denominator stays `distinct - unknown - label_kind` and its ratio is suppressed only by contamination, because whether text survived is knowable regardless of position evidence, so every recorded D3 figure must be unchanged by this task

**Ordering — recorded intent**: **T092 → T093 → T094 → the paid T085 measurement → T095.**
T095 is measurement-only and blocks nothing; it is sequenced after T085 because the run that
motivated it had to happen first, and it needs no further paid run to be verified. T092 and T093
